...
document.querySelector("#btn2a").addEventListener("click",function(){
    ...
    /* 참조란/
        ...
        이 때, 주소를 찾아가는 것을 [참조]라고 한다. 
    */
    arrayTest(arr1);
    arrayTest(arr2);

     console.log(arr2[2]); //딸기
     //왜 멜론이 아니라 딸기가 출력 되는가?
     // -> arrayTest(arr2) 호출 시
...
/* 매개변수로 요소 전달하기 */
function btn2bFn(el){
    //매개 변수 el(element) : 이벤트가 발샐한 요소(HTML에서 this)
...
btn3a.addEventListener("click",function(){
    ...
    numbers.push(50);
    numbers.push(4);
    numbers.push(pow(2,5)); //32 ==number.push(32)
...
arrowList[2].addEventListener("click",()=>{
    ...
    twoNumberPlus( (a,b) =>  a+b ); //return 생략 (a,b) => {return a + b ;}
});
